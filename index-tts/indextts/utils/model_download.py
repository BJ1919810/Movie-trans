...
def _locate_snapshot(repo_id: str, local_cache_dir: str, extra_cache_dirs=None) -> str | None:
    """Search local, project and default HuggingFace caches for *repo_id*."""
    default_hf_cache = os.environ.get(
        "HF_HUB_CACHE",
        os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "hub"),
    )
    search_dirs = [local_cache_dir, *(extra_cache_dirs or [])]
...
def _legacy_dirs(model_dir: str, bigvgan_repo: str) -> dict:
    """历史版本 / 手工部署时常见的本地模型位置（优先复用，避免重复下载）。"""
    bigvgan_name = bigvgan_repo.split("/")[-1]
    return {
        "w2v_bert": [
            os.path.join(model_dir, "w2v-bert-2.0"),
        ],
        "semantic_codec": [
            os.path.join(model_dir, "semantic_codec", "model.safetensors"),
        ],
        "campplus": [
            os.path.join(model_dir, "campplus_cn_common.bin"),
        ],
        "bigvgan": [
            os.path.join(model_dir, "nvidia", bigvgan_name),
            os.path.join(model_dir, bigvgan_name),
            os.path.join(model_dir, "bigvgan"),
        ],
    }


def _extra_cache_dirs(model_dir: str) -> list:
    """额外的 HuggingFace 缓存目录（例如项目根目录下的 checkpoints/hf_cache）。"""
    # model_dir 通常是 <project>/index-tts/checkpoints，项目根缓存是 <project>/checkpoints/hf_cache
    return [os.path.join(model_dir, "..", "..", "checkpoints", "hf_cache")]
...
def ensure_models_available(model_dir: str, bigvgan_repo: str = _BIGVGAN_REPO) -> dict:
    ...
    Lookup order for every auxiliary model (first hit wins):
        1. already-present flat layout in ``{model_dir}/hf_cache/``
        2. legacy local locations (``{model_dir}/w2v-bert-2.0``, ``{model_dir}/nvidia/...`` 等)
        3. old standard HuggingFace Hub cache layout
           (``models--{owner}--{name}/snapshots/{hash}/``) — copied into the flat layout
        4. download (ModelScope / hf-mirror / HuggingFace，自动选路)
    ...
    paths = {}
    legacy = _legacy_dirs(model_dir, bigvgan_repo)
    extra_caches = _extra_cache_dirs(model_dir)
    ...
    if not os.path.isdir(w2v_dir) or not os.listdir(w2v_dir):
        legacy_dir = _first_existing(legacy["w2v_bert"])
        if legacy_dir:
            print(f">> Reusing local w2v-bert-2.0: {legacy_dir} -> {w2v_dir}")
            shutil.copytree(legacy_dir, w2v_dir, dirs_exist_ok=True)
        else:
            old_snapshot = _locate_snapshot("facebook/w2v-bert-2.0", cache_dir, extra_caches)
            if old_snapshot:
    ...
    for key, repo_id, remote_file, local_file, label in (
        ...
        if not os.path.isfile(local_file):
            legacy_file = _first_existing(legacy[key])
            if legacy_file:
                print(f">> Reusing local {label}: {legacy_file}")
                shutil.copy2(legacy_file, local_file)
            else:
                old_snapshot = _locate_snapshot(repo_id, cache_dir, extra_caches)
    ...
    if missing_bigvgan:
        legacy_dir = _first_existing(legacy["bigvgan"])
        if legacy_dir and all(os.path.isfile(os.path.join(legacy_dir, f)) for f in bigvgan_files):
            print(f">> Reusing local BigVGAN: {legacy_dir} -> {bigvgan_dir}")
            os.makedirs(bigvgan_dir, exist_ok=True)
            for fname in bigvgan_files:
                shutil.copy2(os.path.join(legacy_dir, fname), os.path.join(bigvgan_dir, fname))
        else:
            old_snapshot = _locate_snapshot(bigvgan_repo, cache_dir, extra_caches)
